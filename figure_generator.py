...
try:
    df = pd.read_csv('Olympics_Data_Normaliziation.csv')
...
# --- FIGURE 1: THE ULTIMATE LEADERBOARD (FULL PRECISION FIX) ---
plt.figure(figsize=(16, 12))
df_sorted = df.sort_values('Normalized100m')

# We use the 'ax' object to access containers correctly
ax1 = sns.barplot(
    x='Normalized100m', 
    y='Name', 
    data=df_sorted, 
...
# FIX: Loop through all containers to add labels. 
# This prevents the 'NoneType' error caused by the 'hue' grouping.
for container in ax1.containers:
    # Extract the actual value for the label to maintain full precision
    # We use the x-value of the bar directly
    ax1.bar_label(
        container, 
        fmt='%.4f s', # Change to .6f if you need even more digits
        padding=15, 
        color='white', 
        fontweight='bold', 
        fontsize=14
...
# Add space on the right for the long decimal strings
...
# --- FIGURE 3: WIND vs. NORMALIZATION ---
plt.figure(figsize=(10, 6))
sns.regplot(x='Wind', y='Normalized100m', data=df, scatter_kws={'s':100}, line_kws={'color':'red'})
plt.title("How Wind Speed Shifts Normalized Times")
plt.savefig('03_wind_impact.png')

# --- FIGURE 4: ALTITUDE vs. NORMALIZATION ---
plt.figure(figsize=(10, 6))
sns.scatterplot(x='Altitude', y='Normalized100m', size='Wind', hue='Event', data=df, palette='Set2', sizes=(50, 400))
plt.title("Altitude vs Performance (Bubble Size = Wind)")
plt.savefig('04_altitude_impact.png')

# --- FIGURE 5: EVENT EFFICIENCY DECAY ---
plt.figure(figsize=(10, 6))
sns.boxplot(x='Event', y='Normalized100m', data=df, hue='Event', palette='viridis', legend=False)
...
img6 = ax6.scatter(df['Wind'], df['Altitude'], df['Normalized100m'], c=df['Normalized100m'], cmap='plasma', s=200)
...
plt.hlines(y=df_delta['Name'], xmin=0, xmax=df_delta['Delta'], color=colors, alpha=0.8, linewidth=5)
...
plt.figure(figsize=(10, 8))
hb = plt.hexbin(df['ReactionTime'], df['Normalized100m'], gridsize=15, cmap='magma', mincnt=1)
...
# --- FIGURE 13: CATEGORICAL SWARM ---
df['Wind_Type'] = pd.cut(df['Wind'], bins=[-10, -0.1, 0.1, 10], labels=['Headwind', 'Neutral', 'Tailwind'])
plt.figure(figsize=(12, 7))
sns.swarmplot(x='Wind_Type', y='Normalized100m', hue='Event', data=df, size=9, palette='cool')
plt.title('Normalization Efficiency across Wind Conditions')
plt.grid(axis='y', alpha=0.2)
plt.savefig('13_wind_swarm.png')

print("All figures generated successfully with exact precision.")